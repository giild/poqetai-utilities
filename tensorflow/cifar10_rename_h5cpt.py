...
def run(inputfile):
    ...
    modelfilename = inputfile.replace("\\","/")
    ...
    testvalidate = model.evaluate(ds_test, batch_size=128)
    ...
    result.starttime = time.time()
    # iterate over tensorflow dataset
    trainvalidate = model.evaluate(ds_train, batch_size=128)
    result.endtime = time.time()
    elapsed = result.endtime - result.starttime
    print(' time: ', elapsed/60, ' minutes')
    print(trainvalidate)
    print(' ------------------- done -------------------')
    newName = formatFilename(modelfilename, trainvalidate, testvalidate)
    renameCheckpointfile(modelfilename, newName)
    
def formatFilename(inputfilename, trainval, testval):
    tokens = inputfilename.split("-")
    tracc = f"{trainval[1]:.3f}"
...
def renameCheckpointfile(inputname, newname):
    print(' renamging checkpoint ', inputname, " to new name - ", newname)
