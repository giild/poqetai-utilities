...
def main(args):
    if len(args) == 1:
        ...
        modified.save('pruned_checkpoint_model.h5')
...
def modifyWeights(wcorrections, model):
    ...
    for c in range(clen):
        ...
        cweight = float(crt["correctWeight"])
        ...
        if isinstance(layr, tf.keras.layers.Dense) and wttype == "p":
            ...
            if conv2dmode == 'all':
                ...
                lweights[idx1, idx2, idx3,  idx4] = 0.0
                convChangeCount += 1
            # not all Conv2D layers have bias, so we have to check the weight length
            if len(layr.weights) == 2:
                layr.set_weights(np.array([lweights, bweights], dtype=object))
            else:
