...
def load_checkpoint(checkpoint_path: str) -> Dict[str, torch.Tensor]:
    """
    Load a checkpoint file in safetensors format with sorted keys.
    
    Args:
        checkpoint_path: Path to the safetensors checkpoint file
        
    Returns:
        OrderedDict mapping layer names to tensors in sorted order
    """
...
def run(checkpoint1_path: str, checkpoint2_path: str, model_name: str) -> None:
    ...
    # Save to JSON file
    output_filename = f"{model_name}_unchanged_weights_analysis.json"
